...
void ScoreSeqSet::calcPvalues( std::vector<std::vector<float>> pos_scores,
    ...
    for (size_t n = 0; n < seqN; n++) {
        ...
        posN += pos_scores[n].size();
    }

    std::cout << "Total number of positions is " << posN << std::endl;
    bool run_slow = true;
        ...
        for (size_t n = 0; n < seqN; n++) {
            ...
            for (size_t i = 0; i < LW1; i++) {
                ...
                if (FPl == negN) {
                    ...
                    pVal = float(nTop) / (float) negN * expf(-(Sl - S_ntop) / lambda);
                    ...
                    pVal = ((float) FPl + (SlHigher - Sl + eps) / (SlHigher - SlLower + eps))
                              / (float) negN;
