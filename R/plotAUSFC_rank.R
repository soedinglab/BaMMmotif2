...
# calculate FDR, precision, recall and occurrences for ZOOPS
# and MOPS (optional) models by taking p-values of log odds scores from
# positive sequences and using "fdrtool" library;
# plot FDR vs. recall(sensitivity) curve (optional);
# caluclate the area under the sensitivity-FDR curve (AUSFC).

# examples for running this script:
# ./plotAUSFC_benchmark_fdrtool.R PATH_TO_zoops.stats_FILE BASENAME_OF_THE_FILE PATH_OF_OUTPUT_FILE [--mops] [--plot]
# ./plotAUSFC_benchmark_fdrtool.R /home/bamm_result/ JunD /home/bamm_result/output.txt

# If --mops is applied, the mops.pvalue file is required.
# If --plot is applied, a plot of FDR-recall curve will be generated.
...
parser$add_argument('--mops', action="store_true", default='False')
...
if( args$mops != 'False' ){ 
  results = c(paste(c("prefix", "motifNumber", "ausfc_zoops*occ_frac", "occ_frac", "ausfc_mops*occ_mult", "occ_mult"), collapse="\t"))
} else {
  results = c(paste(c("prefix", "motifNumber", "ausfc_zoops*occ_frac", "occ_frac"), collapse="\t"))
}
for (f in Sys.glob(paste(c(dir, "/", prefix, "_motif_*", ".zoops.pvalues"), collapse=""))) {
  ...
  if( args$mops != 'False' ){
    resultString = paste(c(prefix, motifNumber, ausfc_zoops*occ_frac, occ_frac, ausfc_mops*occ_mult, occ_mult), collapse="\t")
  } else {
    resultString = paste(c(prefix, motifNumber, ausfc_zoops*occ_frac, occ_frac), collapse="\t")
  }
  print(resultString)
