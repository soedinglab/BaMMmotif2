#!/usr/bin/env Rscript
# Title     : calculate AUSFC score for reranking PWMs from PEnG!motif
# Objective : calculate AUSFC score for reranking PWMs from PEnG!motif
# Created by: wanwan
# Created on: 29.09.17
...
for (f in Sys.glob(paste(c(dir, "/", prefix, "*", ".zoops.stats"), collapse=""))) {
    ...
    #       using the raw values from BaMM
    #
    ###################################################################
    raw_fdr         <- stats$V3
    raw_recall      <- stats$V4
